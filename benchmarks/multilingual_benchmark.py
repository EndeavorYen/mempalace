...
LANGUAGE_DETECTION_CASES = [
    ...
    TestCase("The microservice architecture uses gRPC for inter-service communication.", "en", ["en"]),

    # === French (Français) ===
    TestCase("Le code a un bug dans la base de données. Il faut corriger.", "en", ["fr"]),
    TestCase("Nous avons décidé de migrer vers PostgreSQL pour de meilleures performances.", "en", ["fr"]),
    TestCase("L'architecture du système utilise des microservices déployés dans Docker.", "en", ["fr"]),
    TestCase("Le module d'authentification gère les sessions avec des jetons JWT.", "en", ["fr"]),

    # === Spanish (Español) ===
    TestCase("El sistema tiene un error en la base de datos que necesita corrección.", "en", ["es"]),
    TestCase("Decidimos migrar la arquitectura a microservicios para mejorar la escalabilidad.", "en", ["es"]),
    TestCase("La planificación del proyecto incluye tres fases con plazos definidos.", "en", ["es"]),

    # === German (Deutsch) ===
    TestCase("Der Code hat einen Fehler in der Datenbankabfrage. Wir müssen debuggen.", "en", ["de"]),
...
ENTITY_DETECTION_CASES = [
    ...
    TestCase("Simon和团队讨论了问题。Simon提出了方案。Simon又补充了几点。", "Simon", ["en", "mixed"]),
...
ROOM_CLASSIFICATION_CASES = [
    # Chinese technical
    TestCase("我们需要修改代码来修复这个错误。调试了很久终于找到问题。部署到服务器上。", "technical", ["zh"]),
    TestCase("函数的接口需要重构，数据库的查询也要优化。", "technical", ["zh"]),

    # Chinese architecture
    TestCase("系统的架构设计需要重新考虑。模块和组件的结构不够合理。服务之间的依赖太复杂。", "architecture", ["zh"]),

    # Chinese planning
    TestCase("我们需要制定计划，确定里程碑和截止日期。需求规格要尽快完成。优先级需要重新排列。", "planning", ["zh"]),

    # Chinese decisions
    TestCase("我们决定使用新的方案。选择了这个策略是因为权衡了各种因素。替换旧的方法。", "decisions", ["zh"]),

    # Chinese problems
    TestCase("系统出现了严重的故障，崩溃了好几次。这个问题需要尽快修复和解决。", "problems", ["zh"]),

    # Traditional Chinese
    TestCase("我們需要修改代碼來修復這個錯誤。調試了很久終於找到問題。", "technical", ["zh", "traditional"]),
    TestCase("我們決定使用新的方案。選擇了這個策略是因為權衡了各種因素。", "decisions", ["zh", "traditional"]),

    # English (baseline)
    TestCase("The code has a bug in the database query function. Need to debug the API server.", "technical", ["en"]),
    TestCase("We decided to switch from MySQL to PostgreSQL. The trade-off was worth it.", "decisions", ["en"]),
    ...
    TestCase("Le code a un bug dans la base de données. Il faut déboguer le serveur API.", "technical", ["fr"]),
    TestCase("Nous avons décidé de migrer vers PostgreSQL. Le compromis en vaut la peine.", "decisions", ["fr"]),
    TestCase("Le système a planté. Une erreur critique a causé une panne du serveur.", "problems", ["fr"]),
    TestCase("Nous devons planifier les étapes du projet et fixer les délais.", "planning", ["fr"]),
    TestCase("L'architecture du système utilise des microservices avec des conteneurs Docker.", "architecture", ["fr"]),
    TestCase("Nous avons passé une belle journée au parc aujourd'hui.", "general", ["fr", "general"]),

    # === Spanish (embedding-based) ===
    TestCase("El código tiene un error en la consulta de base de datos. Necesitamos depurar el servidor API.", "technical", ["es"]),
    TestCase("Decidimos cambiar de MySQL a PostgreSQL por mejor soporte JSON. Fue una decisión difícil después de evaluar las alternativas.", "decisions", ["es"]),
    TestCase("El sistema falló con un error crítico. El servidor se cayó varias veces.", "problems", ["es"]),
    TestCase("Debemos planificar las fases del proyecto y establecer los plazos.", "planning", ["es"]),
    TestCase("La arquitectura del sistema utiliza microservicios con contenedores Docker.", "architecture", ["es"]),

    # === German (embedding-based) ===
    TestCase("Der Code hat einen Fehler in der Datenbankabfrage. Wir müssen den API-Server debuggen.", "technical", ["de"]),
    TestCase("Wir haben uns entschieden, von MySQL auf PostgreSQL umzusteigen.", "decisions", ["de"]),
    TestCase("Das System ist mit einem kritischen Fehler abgestürzt. Der Server fiel mehrmals aus.", "problems", ["de"]),
    TestCase("Wir müssen die Projektphasen planen und die Termine festlegen.", "planning", ["de"]),

    # === Japanese (embedding-based) ===
    TestCase("データベースクエリにバグがあります。APIサーバーをデバッグする必要があります。", "technical", ["ja"]),
    TestCase("MySQLからPostgreSQLに移行することにしました。トレードオフは価値がありました。", "decisions", ["ja"]),
    TestCase("システムが重大なエラーでクラッシュしました。サーバーが数回ダウンしました。", "problems", ["ja"]),
]
...
MEMORY_EXTRACTION_CASES = [
    # Chinese decisions
    TestCase("经过讨论，我们决定使用 GraphQL 而不是 REST API。这个方案更适合我们的需求，因为更灵活。", "decision", ["zh"]),
    TestCase("我們決定使用微服務架構，選擇了這個策略因為權衡了擴展性。", "decision", ["zh", "traditional"]),

    # Chinese preferences
    TestCase("我偏好函数式编程风格，总是用不可变数据结构。千万不要用全局变量，这是我的习惯。", "preference", ["zh"]),

    # Chinese milestones
    TestCase("终于成功了！经过三天的努力，我们实现了完整的搜索功能。这是一个重大突破，第一次做到这样。", "milestone", ["zh"]),
...
SEARCH_CORPUS = [
    ("doc_zh_tech_1", "认证模块使用JWT令牌进行会话管理。令牌在24小时后过期。刷新令牌存储在HttpOnly的cookie中。"),
    ("doc_zh_tech_2", "数据库迁移由Alembic处理。我们使用PostgreSQL 15作为主要数据库。"),
    ("doc_zh_arch_1", "系统架构采用微服务设计，每个服务独立部署在Docker容器中。网关层使用Nginx做负载均衡，服务间通过gRPC通信。"),
    ...
    ("doc_fr_tech_1", "Le module d'authentification utilise des jetons JWT pour la gestion des sessions."),
    ("doc_fr_arch_1", "Le système utilise une architecture de microservices avec des conteneurs Docker."),
    ("doc_es_tech_1", "El módulo de autenticación utiliza tokens JWT para la gestión de sesiones."),
    ("doc_de_tech_1", "Das Authentifizierungsmodul verwendet JWT-Token für die Sitzungsverwaltung."),
...
def _run_search_benchmark() -> DimensionScore:
    ...
    try:
        ...
        return DimensionScore(name="Search Quality", score=0, passed=0, total=len(SEARCH_QUERIES),
    ...
    score = (passed / total * 100) if total > 0 else 0
    failures = [r for r in results if not r.passed]
    return DimensionScore(name="Search Quality", score=score, passed=passed, total=total,
                          failures=failures, duration_ms=duration)
...
def _run_opencc_benchmark() -> DimensionScore:
    ...
    try:
        ...
        return DimensionScore(name="OpenCC Consistency", score=0, passed=0,
    ...
    for simplified, traditional, desc in OPENCC_CONSISTENCY_PAIRS:
        ...
        results.append(TestResult(
            ...
            actual=f"s={room_s},t={room_t}", passed=passed,
        ...
        results.append(TestResult(
            ...
            actual=room_converted, passed=passed,
        ...
        results.append(TestResult(
            ...
            actual=f"s={type_s},t={type_t}", passed=passed,
    ...
    failures = [r for r in results if not r.passed]
    return DimensionScore(name="OpenCC Consistency", score=score, passed=passed_count,
...
def run_benchmark(verbose: bool = False, dimensions_filter: str = None):
    ...
    dims_to_run = {
        "language_detection": ("Language Detection", LANGUAGE_DETECTION_CASES, _run_language_detection),
        "entity_detection": ("Entity Detection", ENTITY_DETECTION_CASES, _run_entity_detection),
        "room_classification": ("Room Classification", ROOM_CLASSIFICATION_CASES, _run_room_classification),
